...
7. Оценка 8-10 ставится ТОЛЬКО для хорошо освещенных портретов высокого качества по пояс (medium shot) или крупнее, где лицо анфас (допускается лёгкий поворот головы), крупное, глаза открыты и ничем не перекрыты (кроме очков).
Верни строго сырой JSON без маркдауна и других слов (ничего кроме JSON):
{
  ...
  "qualityMessage": "короткий комментарий на русском описывающий главную причину оценки" 
        ...
        return { 
            statusCode: 500, 
            body: JSON.stringify({ isAllowed: true, score: 0, qualityMessage: "Ошибка (серверная проверка пропущена)" }) 
        };
    }
};
